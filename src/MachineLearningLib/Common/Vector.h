...
namespace FengML
{
    ...
    Vector<T>& Vector<T>::AssignMul(const Matrix<T>& W, const Vector<T>& v)
    {
        ...
        T* wdata = W.m_data;
        T* data = m_data;
        for (int i = 0; i < m_Len; i++, data++)
        {
            T* vdata = v.m_data;
            T sum = 0;
            for (int j = 0; j < W.col; j++, vdata++, wdata++)
            {
                sum += *wdata * *vdata;
            }
            *data = sum;
    ...
    Vector<T>& Vector<T>::AssignMulTMat(const Matrix<T>& W, const Vector<T>& v)
    {
        ...
        *this = 0;
        ...
        T* data = m_data;
        T* vdata = v.m_data;
        for (int i = 0; i < W.row; i++, vdata++)
        {
            for (int j = 0; j < W.col; j++, wdata++)
            {
                data[j] += *wdata * *vdata;
            }
        }
    ...
    template<class T>
    std::pair<T, int> Vector<T>::Max() const
    {
        auto res = std::make_pair<T, int>(0, -1);
        if (m_Len > 0)
        {
